...
export const processSingleBatch = async (databaseName: string, tablesToMonitor: string[], binlogCheckpointName: string, minDurationInSeconds: number, maxDurationInSeconds: number): Promise<BatchOrchestrationResult> => {
  ...
  try {
    ...
    const batchResults = await getBinlogBatch(connection, checkpoint.binlogName, checkpoint.binlogPosition, tablesToMonitor, databaseName, minDurationInSeconds, maxDurationInSeconds);
